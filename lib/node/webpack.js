...
const envIsTest = function() {
  return process.env.TEST === "1";
};

const envIsWatch = function() {
  return process.env.WATCH === "1";
};

const envIsProd = function() {
...
const getDefinePlugin = ({ isNode, env, nameSpaceId, isLibrary }) => {
  ...
  let variables = {
    BUILD: {
      ...
      IS_TEST: JSON.stringify(envIsTest()),
      IS_NODE: JSON.stringify(isNode),
      IS_DEV: JSON.stringify(!envIsProd()),
      IS_PROD: JSON.stringify(envIsProd()),
      IS_WATCH: JSON.stringify(envIsWatch())
    }
  };
